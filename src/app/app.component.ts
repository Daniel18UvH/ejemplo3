...
agreProducto(){
  ...
  //reiniciamos el objeto prdocuto a sus valores iniciales
  this.producto.id=0;
  ...
  selecProducto(productoseleccionado: {
    id:number; descripcion:string;precio:number}){
  ...
  modiProducto(){
   for(let i=0;i< this.productos.length; i++){
    if(this.producto.id == this.productos[i].id){
      ...
      //reserteamos el objeto producto
      this.producto.id = 0;
      this.producto.descripcion = '';
      this.producto.precio = 0;
      return;
   ...
   alert('No existe este ID'); //este mensaje es en caso de que no exisra nuestra tabla
  }
